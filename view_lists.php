...
<body id="list_body">
    ...
    <main>
        ...
        <div class="view-a-list col-md-12 table-responsive">
            <table class="table">
                <tr>
                    <th>Name</th>
                    <th>Purchase day</th>
                ...
                $sql = "SELECT l.* FROM list l LEFT JOIN customer c ON c.id_customer = l.id_customer WHERE c.email = '".$_SESSION['username']."' ";
                    $q = $pdo->query($sql);
                    ...
                    <tr class="<?php echo ($row['id_list'])?>"><td><?php echo ($row['name'])?></td><td><?php echo ($row['purchase_day']) ?></td><td><?php echo ($row['description']) ?></td><td><?php echo ($row['date_created']) ?></td><td><?php echo ($row['status']) ?></td><td><span id="<?php echo ($row['id_list'])?>" data-name="<?php echo $row['name'] ?>" class="remove-list btn btn-danger">Remove</span></td><td><span id="<?php echo ($row['id_list'])?>" class="view_list btn btn-success">Check list</span></td></tr>
