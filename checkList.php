...
<body id="list_body">
    ...
    <main>
        ...
        <div class="view-a-list checkList col-md-12 align-center">
            <table>
                <tr>
                    <th>ID Item</th>
                    <th>Name</th>
                ...
                <?php
                    if (isset($_GET["id_list"]) and is_numeric($_GET['id_list'])) {
                        $l = (int)$_GET["id_list"];
                        echo '<div class="list-item col-12 col-sm-12 col-md-12">';

                        $sql = "
                            SELECT li.*, i.name, l.status AS state FROM list_items li 
                            LEFT JOIN items i ON li.id_item = i.id_item 
                            LEFT JOIN list l ON li.id_list = l.id_list 
                            WHERE li.id_list = $l ";
                        $q = $pdo->query($sql);
                        ...
                        while($row = $q -> fetch()) {
                        ?>
                            <tr class="<?php echo ($row['id_item'])?>"><td><?php echo ($row['id_item'])?></td><td><?php echo mb_strimwidth($row['name'], 0, 15, "...");?></td><?php if($row['state'] != 'finished') { ?><td><input type="checkbox" class="checkListBox" name="checkListBox" id="<?php echo $row['id_item'] ?>" name="<?php echo $row['id_item'] ?>" value="0"></td><?php } else { ?> <td>Item has been purchased!</td <?php } ?></tr>
            ...
            $sql = "
                SELECT l.id_list, l.status FROM list l 
                WHERE l.id_list = $l ";
                $q = $pdo->query($sql);
                $q -> setFetchMode(PDO::FETCH_ASSOC);
            ...
            ?>
                <span onClick="finishList(<?php echo $l ?>)" class="btn btn-success mt-3">Finish!</span>
            <?php }} ?>
